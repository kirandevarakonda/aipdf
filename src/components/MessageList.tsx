...
export default function MessageList({
  ...
  return (
    <div className="w-full px-2 sm:px-4 space-y-3 max-w-3xl mx-auto">
      {messages.map((message) => (
        <div
          key={message.id}
          className={cn(
        ...
        >
          <div
            className={cn(
              "p-3 rounded-lg text-sm shadow-sm relative w-full",
              message.role === "user"
                ? "bg-gray-900 text-white sm:ml-12 md:ml-20"
                : "bg-white text-gray-800 border border-gray-200 sm:mr-12 md:mr-20"
            )}
          >
            <ReactMarkdown
              remarkPlugins={[remarkGfm]}
              components={{
                ...
                p: ({ node, ...props }) => (
                  <p className="mb-1 last:mb-0 text-sm" {...props} />
                ),
                a: ({ node, ...props }) => (
                  <a className="text-blue-600 hover:underline break-words" {...props} />
                ),
                blockquote: ({ node, ...props }) => (
                  <blockquote className="pl-3 border-l-3 border-gray-300 text-gray-700 my-1 text-sm" {...props} />
                ),
                h1: ({ node, ...props }) => (
                  <h1 className="text-lg font-bold my-1" {...props} />
                ),
                h2: ({ node, ...props }) => (
                  <h2 className="text-base font-bold my-1" {...props} />
                ),
                h3: ({ node, ...props }) => (
                  <h3 className="text-sm font-bold my-1" {...props} />
                ),
              }}
              className="prose-sm prose-gray max-w-none break-words"
            >
              {message.content}
            </ReactMarkdown>
            
            <div className="flex items-center justify-between mt-1 pt-1 text-xs">
              <div
                className={cn(
                  "text-[10px]",
                  message.role === "user" ? "text-indigo-100" : "text-gray-500"
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          ...
                          size="sm"
                          onClick={() => handleCopy(message.content)}
                          className="h-6 w-6 p-1 text-gray-500 hover:bg-gray-100 rounded-full"
                        >
                          <Copy className="h-3 w-3" />
                        </Button>
                      </TooltipTrigger>
      ...
      {isLoading && (
        <div className="w-full max-w-[95%] mx-auto">
          <div className="bg-white border border-gray-200 rounded-lg p-3 text-sm shadow-md animate-pulse">
